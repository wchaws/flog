...
const (
	...
	JSONLogNK             = `{"no": "%d", "level":"%s","time":"%s","logger":"gateway","message":"%s","app_name":"gateway","code":0,"file":"log.go:75","module_name":"m","function":"func","hostname":"%s","env":"testing","request_id":"%s","method":"%s","url":"%s","status_code":%d,"content_length":%d,"user_agent":"%s","elapsed":%d,"remote_ip":"%s","client_ip":"%s"}`
